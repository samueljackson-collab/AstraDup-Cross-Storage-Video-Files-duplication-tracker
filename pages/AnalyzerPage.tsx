                    ...
                    {sources && sources.length > 0 && (
                         <div className="mt-6 border-t border-green-800 pt-4">
                            ...
                            <ul className="space-y-2">
                                {sources.map((source: Record<string, { uri?: string; title?: string }>, index: number) => (
                                    source.web && (
...
const ImageAnalyzer = () => {
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div>
                {file && previewUrl ? <img src={previewUrl} alt="preview" className="rounded-lg w-full aspect-video object-contain bg-black" /> : <Dropzone onDrop={setFile} accept="image/*" fileType="image" />}
...
const VideoAnalyzer = () => {
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div>
                {file && videoUrl ? <video ref={videoRef} src={videoUrl} controls className="rounded-lg w-full aspect-video object-contain bg-black" crossOrigin="anonymous" preload="metadata"/> : <Dropzone onDrop={setFile} accept="video/*" fileType="video" />}
